...
export class WalletsService {
  ...
  private async findBalance(address: string): Promise<number> {
    const response = await fetch(process.env.WALLET_API_URL.replace('{address}', address));
  ...
  private async findTransactions(address: string): Promise<any[]> {
    const response = await fetch(process.env.TRANSACTIONS_API_URL.replace('{address}', address));
    const data = await response.json();
    if (data.status === '1') {
      return data.result;
    } else {
  ...
  async create(userId: string, address: string, usd: number, eur: number): Promise<Wallet> {
    ...
    const isOld = oldestTransaction && new Date(+oldestTransaction.timeStamp) < new Date(Date.now() - 365 * 24 * 60 * 60 * 1000);
